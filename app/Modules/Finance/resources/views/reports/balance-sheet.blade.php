...
<x-app-layout>
    <x-slot name="header">
        <span class="text-ink-black dark:text-paper-white">Laporan Neraca Keuangan (Balance Sheet)</span>
    </x-slot>
    ...
    <div class="max-w-5xl mx-auto space-y-6">
        ...
        <div class="bg-paper-white dark:bg-[#111c44] border border-border-gray/80 dark:border-border-gray/10 rounded-card p-4 shadow-subtle flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div class="hidden lg:flex items-center gap-2 text-caption text-slate-gray dark:text-ash-gray">
                ...
                <span>Posisi Saldo Per: <strong class="text-ink-black dark:text-paper-white">{{ $report['as_of_date']->translatedFormat('d F Y') }}</strong></span>
            </div>
        </div>

        {{-- Unbalanced Alert Banner --}}
        @unless($report['is_balanced'])
            <div class="rounded-card bg-danger-bg dark:bg-danger/10 border border-danger/30 text-danger p-4 shadow-subtle flex items-start gap-3">
        ...
        <div class="bg-paper-white dark:bg-[#111c44] border border-border-gray/80 dark:border-border-gray/10 rounded-card shadow-subtle overflow-hidden">
            ...
            <div class="p-6 space-y-8">
                ...
                <div>
                    ...
                    <div class="divide-y divide-border-gray/40 dark:divide-border-gray/10">
                        ...
                        @empty
                            <p class="text-caption text-ash-gray dark:text-slate-gray py-3 px-2 italic">Tidak ada pos aset tercatat.</p>
                        @endforelse
                    </div>
                    ...
                    <div class="flex justify-between items-center px-2 pt-3 mt-2 border-t border-border-gray dark:border-border-gray/10 text-body-sm font-bold text-ink-black dark:text-paper-white bg-fog-white/40 dark:bg-paper-white/5 rounded-input">
                        <span class="text-slate-gray dark:text-ash-gray">Total Aset (Aktiva)</span>
                ...
                <div>
                    <div class="flex items-center justify-between pb-2 mb-3 border-b border-border-gray/60 dark:border-border-gray/10">
                        <span class="text-[11px] font-bold text-[#c2410c] dark:text-[#ea580c] uppercase tracking-wider flex items-center gap-1.5">
                            <x-dynamic-component component="lucide-receipt" class="w-3.5 h-3.5 text-[#c2410c] dark:text-[#ea580c]" />
                    ...
                    <div class="divide-y divide-border-gray/40 dark:divide-border-gray/10">
                        ...
                        @empty
                            <p class="text-caption text-ash-gray dark:text-slate-gray py-3 px-2 italic">Tidak ada pos liabilitas tercatat.</p>
                        @endforelse
                    </div>

                    <div class="flex justify-between items-center px-2 pt-3 mt-2 border-t border-border-gray dark:border-border-gray/10 text-body-sm font-bold text-ink-black dark:text-paper-white bg-fog-white/40 dark:bg-paper-white/5 rounded-input">
                        <span class="text-slate-gray dark:text-ash-gray">Total Liabilitas (Kewajiban)</span>
                ...
                <div>
                    ...
                    <div class="divide-y divide-border-gray/40 dark:divide-border-gray/10">
                        ...
                        <div class="flex justify-between items-center py-2.5 hover:bg-mist-gray/30 dark:hover:bg-paper-white/5 px-2 rounded-input transition-colors bg-primary/10 dark:bg-primary/20">
                            <div class="flex items-center gap-2">
                                <span class="font-mono text-caption font-semibold text-primary bg-primary/20 dark:bg-primary/30 px-2 py-0.5 rounded-input border border-primary/30">
                                    P/L
                                </span>
                                <span class="text-body-sm font-semibold text-ink-black dark:text-paper-white">
                                    Laba (Rugi) Berjalan <span class="text-caption font-normal text-slate-gray dark:text-ash-gray">(Belum Ditutup)</span>
                                </span>
                            </div>
                    ...
                    <div class="flex justify-between items-center px-2 pt-3 mt-2 border-t border-border-gray dark:border-border-gray/10 text-body-sm font-bold text-ink-black dark:text-paper-white bg-fog-white/40 dark:bg-paper-white/5 rounded-input">
                        <span class="text-slate-gray dark:text-ash-gray">Total Ekuitas Bersih</span>
            ...
            <div class="flex flex-col sm:flex-row justify-between items-center px-6 py-5 border-t-2 border-border-gray/80 dark:border-border-gray/20 bg-fog-white/90 dark:bg-paper-white/5 gap-2">
                <div>
                    <span class="text-body font-bold text-ink-black dark:text-paper-white block leading-tight">Total Pasiva (Liabilitas + Ekuitas)</span>
                    <span class="text-caption text-slate-gray dark:text-ash-gray">Total Claims on Assets</span>
                </div>

                <span class="text-heading font-bold tabular-nums text-ink-black dark:text-paper-white">
