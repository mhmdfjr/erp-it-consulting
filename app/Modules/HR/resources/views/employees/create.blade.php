<x-app-layout>
    ...
    <form method="POST" action="{{ route('hr.employees.store') }}" class="max-w-2xl space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
            <x-input name="employee_code" label="Kode Employee" required :value="old('employee_code') ?? ''" />
            <x-input name="full_name" label="Nama Lengkap" required :value="old('full_name') ?? ''" />
        </div>

        <div class="grid grid-cols-2 gap-4">
            <x-input name="nik" label="NIK" :value="old('nik') ?? ''" />
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <x-input name="birth_date" type="date" label="Tanggal Lahir" required :value="old('birth_date') ?? ''" />
        </div>
        ...
        <div>
            ...
            <select name="ptkp_status" class="w-full rounded-input border-border-gray">
                @foreach (['TK0', 'TK1', 'TK2', 'TK3', 'K0', 'K1', 'K2', 'K3'] as $status)
                    <option value="{{ $status }}" {{ old('ptkp_status') === $status ? 'selected' : '' }}>{{ $status }}</option>
                @endforeach
            </select>
        </div>

        <x-input name="base_salary" type="number" step="0.01" label="Base Salary" required :value="old('base_salary') ?? ''" />

        <div class="grid grid-cols-2 gap-4">
            <x-input name="hire_date" type="date" label="Tanggal Bergabung" required :value="old('hire_date') ?? ''" />
            <x-input name="termination_date" type="date" label="Tanggal Berhenti (opsional)" :value="old('termination_date') ?? ''" />
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
            <x-input name="bank_name" label="Nama Bank" :value="old('bank_name') ?? ''" />
            <x-input name="bank_account_number" label="Nomor Rekening" :value="old('bank_account_number') ?? ''" />
        </div>

        <div class="grid grid-cols-2 gap-4">
            <x-input name="phone" label="Telepon" :value="old('phone') ?? ''" />
            <x-input name="email" type="email" label="Email" :value="old('email') ?? ''" />
        </div>

        <x-input name="address" label="Alamat" :value="old('address') ?? ''" />
