...
**Keputusan M3 tambahan (accrual, agregat, aksi Mark as Paid terpisah)**: berbeda dari `SalesOrderCompleted` yang 1:1 dengan `Invoice` per sales order, `PayrollProcessed` di-fire **sekali** setelah seluruh employee di satu `payroll_period` selesai diproses (bukan sekali per `payroll_run`/employee), dan listener `CreateJournalEntryFromPayroll` membuat **satu** journal entry agregat untuk period tersebut, bukan satu JE per employee. Alasan: `journal_entries.reference_type/reference_id` cuma menyimpan satu pasangan reference, dan membuat N journal entry terpisah dalam satu listener execution (N = jumlah employee) menambah risiko partial failure di tengah loop tanpa benefit trace yang proporsional — breakdown per employee tetap tersedia lewat `payroll_run_items`, tidak perlu direplikasi ke `journal_entry_lines`.

Jurnal payroll dibuat **accrual** (saat `PayrollProcessed` diproses, mencatat liability di 202 Utang Gaji), bukan langsung debit kas. Pelunasan net pay ke karyawan adalah aksi UI terpisah ("Mark as Paid" di `PayrollRunController`, task 3.22/3.23) yang toggle `payroll_runs.status` jadi `paid` — mirror pola `VendorBill` di M1 (Section 4, DATABASE.md Section 3.5), **tidak** fire event baru dan **tidak** generate journal entry tambahan di MVP ini (beda dari AR/`payments` yang memang butuh jurnal kedua per payment karena partial payment tracking, lihat Section 4 poin Invoice).
...
| `PayrollProcessed`    | HR module                        | Finance module                       | Generate **satu** `journal_entries` agregat untuk seluruh employee di period tersebut, accrual basis (beban gaji, beban BPJS perusahaan, utang gaji, utang PPh21, utang BPJS gabungan employee+company). Payload cuma `payroll_period_id`, listener query semua `payroll_runs` milik period dan sum per kolom (lihat DATABASE.md Section 2.8a dan Appendix C untuk formula lengkap). Pelunasan net pay ke karyawan (`payroll_runs.status → paid`) aksi terpisah, tidak fire event baru dan tidak generate jurnal tambahan di MVP, konsisten dengan pola `VendorBill`. |
